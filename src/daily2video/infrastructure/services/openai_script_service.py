...
- 書誌情報の各項目は必ず次のフォーマットを守り、先頭のハイフンを省略しないでください（判明している項目だけを出力）
  - `- **タイトル**: ...`（タイトル全体を太字で記載）
  - `  - 著者: ...`
  - `  - 所属: ...`
  - `  - 掲載先・日付: ...`
  - `  - DOI: <...>
  - `  - URL: <...>`
- DOIやURLは必ずMarkdownリンク（`[テキスト](リンク)`）で書き、プレーンURLやカッコ付きURLは避けてください
- 書誌情報は記事本文から抽出できた値のみを使用し、「不明」「なし」などのプレースホルダは書かないでください。項目が無い場合はそのフィールドを省略してください
- 書誌情報リストの直後に空行を1つ入れ、その後に動画の概要とハイライトを200〜400文字で添えてください
- タグを含むすべての出力は日本語で記載し、タグは最大10個、短い語句にしてください
"""
